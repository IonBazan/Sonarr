...
**Stage 3 is where the ticket's actual ask ships**: a multi-season pack added directly to a download client, or grabbed from interactive search, now imports completely instead of being blocked. This is the PR that will get (and deserves) the most scrutiny — it's the one that touches the exact code path #8133 was filed against. Its description should lead with the safety story from §6 edge case 5 (mixed-quality/download-loop risk, now backed by file:line citations into the actual import pipeline) and should mention edge case 6 (queue/history display can legitimately show a different quality than what import history ends up recording) so it doesn't get mistaken for a bug in review. Include the edge case 7 test (mixed-quality import batch) in this PR. Keep this PR to exactly Steps 4 and 6 plus their tests; resist folding in Stage 4 or any UI work.
...
Since this stage is already making the queue season-aware, a reviewer will likely ask whether it should also show a per-season/per-quality breakdown, given §6 edge case 6 (the queue's single `Quality` field can be inaccurate across a mixed-quality pack). Decide this explicitly rather than leaving it implicit: the recommendation is to leave `Quality` as the single existing field for this PR — a real per-season quality breakdown would need the queue to know about individual files before they're imported, which it doesn't today, and that's new scope the ticket didn't ask for. Note it as a known limitation, not a silent gap.
...
- New case (see §6 edge case 7): an import batch with two files of genuinely different actual quality (e.g. one Bluray, one WEB-DL by filename) — assert each `LocalEpisode.Quality` resolves independently from its own filename and isn't influenced by the other file or by the folder/download-item's guessed quality. *(Stage 3)*
...
5. **Mixed-quality packs — data-safety (download-loop concern raised on #8508 by mynameisbogdan)** — e.g. `S01-S09 … Bluray/WEB-DL-Mixed` parses as one release-level `Bluray-1080p` guess, but individual files are genuinely mixed quality. Traced through the actual import pipeline to confirm the safety argument, not just assert it:
   - `AggregationService`/`AggregateQuality` (`MediaFiles/EpisodeImport/Aggregation/Aggregators/AggregateQuality.cs`) computes `LocalEpisode.Quality` **per file**, independently, preferring the file's own filename parse (`Confidence.Tag`) over folder- or download-item-level info; it only falls back to those when the filename itself is unparseable.
   - `UpgradeSpecification` (import-time) compares that per-file `LocalEpisode.Quality` against the actual on-disk `EpisodeFile.Quality` for that specific episode — **not** the release-level guess. A file that's actually WEB-DL will correctly fail the upgrade check against an existing Bluray file and get skipped, regardless of what the pack's title claimed.
   - `HistoryService.Handle(EpisodeImportedEvent)` records that same per-file quality, so "Imported" history rows reflect reality even when they differ from the "Grabbed" row.
   - Net result: **no downgrade/data-loss risk from import itself** — a mixed-quality pack can't silently overwrite a better file. Combined with RSS/automatic search always rejecting multi-season packs, there's no automatic re-grab loop either; a repeat grab is always a deliberate one-off user action. State this explicitly in the Stage 3 PR description, now with the file:line citations above rather than an assertion.
6. **Mixed-quality packs — display accuracy (not a functional bug, but worth calling out)** — the release-level guess described above is what's shown on the **queue row** (`QueueService.MapQueueItem`, `Quality = ...ParsedEpisodeInfo.Quality`) and copied into **every "Grabbed" history row** (`HistoryService.Handle(EpisodeGrabbedEvent)`), for every episode in the release. Before Stage 2, a multi-season pack's `Episodes` list only ever held season 1, so this imprecision only ever touched one season's worth of history/queue rows; after Stage 2 it spans every season in the pack, so the same single guessed quality now gets stamped across potentially years of episodes. Users will see one quality on the queue/grab history and a *different, correct* quality in import history for any season that doesn't match the guess — expected given how quality is resolved, but confusing without a heads-up. No code fix proposed for this (it's how Sonarr already treats every release, single- or multi-season); worth a one-line mention in the Stage 3 warning copy ("actual quality is resolved per episode at import and may differ from what's shown here") and an explicit call-out in that PR's description so it isn't mistaken for a bug in review.
7. **Missing test coverage for mixed-quality imports (Stage 3)** — none of the tests proposed for Stage 3 (`ImportFixture`, `DownloadedEpisodesImportServiceFixture`) exercise a single import batch where files genuinely differ in parsed quality; existing multi-season test cases vary season/episode number but not quality. The underlying per-file resolution is inherited and unchanged, but this is the first stage where a multi-season pack ever reaches that code path in production, so it's worth proving directly rather than relying on inherited coverage of ordinary (usually single-quality) season packs. Add to Stage 3's test list: a `GetImportDecisions`/`ImportFixture` case with two files of different actual quality in one import, asserting each `LocalEpisode.Quality` reflects its own file and `UpgradeSpecification` evaluates each against its own on-disk episode independently.
8. **`AcceptableSizeSpecification`** now sees the true episode count, so per-episode size limits behave correctly for packs (previously it compared a 5-season torrent against 1 season's episode count → large-size rejections).
