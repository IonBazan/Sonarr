...
- *Config toggle "allow multi-season import"* — adds a setting for something that should just work; the per-file import already guards against wrong files (unparseable files are rejected individually and surface as ImportBlocked messages).
...
### Step 1 — `ParsedEpisodeInfo`: `SeasonNumbers` becomes the source of truth
...
### Step 2 — `Parser.cs`: populate `SeasonNumbers`
...
### Step 3 — `ParsingService.GetEpisodes`: resolve across seasons
...
### Step 4 — `MultiSeasonSpecification`: message only
...
### Step 5 — `SeasonMatchSpecification`: season-in-range
...
### Step 6 — `DownloadedEpisodesImportService.ProcessFolder`: remove the gate
...
### Step 6b — Queue model: store season numbers, not a single season
...
### Step 7 — Tests
- `src/NzbDrone.Core.Test/ParserTests/SeasonParserFixture.cs` `should_parse_multi_season_release`: extend test cases with `lastSeason` and assert `result.SeasonNumbers.Should().Equal(Enumerable.Range(first, last-first+1))` and `IsMultiSeason.Should().BeTrue()`. Add a single-season case asserting `SeasonNumbers == [n]` and `IsMultiSeason == false`.
- `src/NzbDrone.Core.Test/DecisionEngineTests/MultiSeasonSpecificationFixture.cs`: unchanged unless it asserts on the message text; if so, update the string.
- New `SeasonMatchSpecificationFixture` case: S01-S05 pack accepted for season 3 search, rejected for season 7.
- `src/NzbDrone.Core.Test/MediaFiles/DownloadedEpisodesImportServiceFixture.cs` ~L518: the test asserting `ImportRejectionReason.MultiSeason` must be inverted — a multi-season download client item should now call `GetImportDecisions` with all video files.
- `ParsingServiceTests`: new fixture asserting `GetEpisodes` for `SeasonNumbers=[1,2,3]`, `FullSeason=true` returns the union of `GetEpisodesBySeason(1..3)` and that a scene offset is applied uniformly.
- `CompletedDownloadServiceFixture`: verify a tracked download whose `RemoteEpisode.Episodes` spans 3 seasons is only marked `Imported` when all imported.
- `PendingReleaseService` / JSON round-trip: serialize a `ParsedEpisodeInfo` with `SeasonNumbers=[1,2,3]` via the same STJ options `EmbeddedDocumentConverter` uses, deserialize, assert array intact; deserialize legacy `{"seasonNumber":2,...}` JSON, assert `SeasonNumbers==[2]`.

### Step 8 — Optional UI polish (small, independent)
- `src/Sonarr.Api.V5/Release/ReleaseResource.cs` / `ParsedEpisodeInfoResource`: expose `seasons` alongside `seasonNumber`.
- `frontend/src/InteractiveSearch/InteractiveSearchRow.tsx` + `ReleaseSceneIndicator`/`EpisodeFormats`: render `S01-S05` for `seasons.length > 1` instead of `Season 1`.
...
## 5. Edge cases to check during implementation
...
- [ ] `Parse` page (Settings → Parse) shows season range for a multi-season title.

## 7. Relationship to prior PRs #8508 and #8668
...
|---|---|---|---|
| `SeasonNumber` + `SeasonNumbers` side by side is confusing; derive one from the other | ✗ both stored | ✗ both stored | ✓ Step 1 facade, `IsMultiSeason` computed |
| Queue model should store season numbers, not one | ✗ | ✗ | ✓ Step 6b |
...
| No "experimental features" toggle | n/a | ✗ | ✓ none |
| New `ReleaseType.MultiSeasonPack` | added | added | not added |
| New parser regexes | none | added | none |
| Discuss before building (CONTRIBUTING.md) | not done | not done | see §9 |
...
Practical consequence: the maintainers asked #8508's author to "comment here and we will reopen this one — please do not open a new PR". The cleanest route is to revive #8508 with the changes above (or coordinate with its author), not to open a third PR cold.

## 8. Follow-ups (not in this PR)

- Accept multi-season packs in interactive search: gate `MultiSeasonSpecification` on `information.SearchCriteria is { InteractiveSearch: true } && subject.Episodes.Any()`. One-line change plus fixture cases; do only after the import path has been exercised by users.
- Optional setting to allow multi-season packs in automatic season/series search.
- Season-range display in queue/history resources.

## 9. Contribution notes (from `AGENTS.md` / `CONTRIBUTING.md`)

The maintainers require a human to own every line: the agent should produce the diff and a plain PR description, but the human must review, commit, push and open the PR — no AI attribution footers. Before coding: open (or comment on) an issue describing this exact scope — CONTRIBUTING.md asks for discussion first and both prior PRs were told so. Reference issue #3678 ("Multiple season import support"), #8133 (data-loss report that motivated the import gate), and PRs #8508 / #8668 in the write-up, and explicitly address the mixed-quality download-loop concern from #8508.
